...
knitr::opts_chunk$set(cache=FALSE, fig.path='figures/', fig.width=8, fig.height=6.5 )
```
...
## Are individual Arth-Symph ASVs present in all depth layers?

Merge by depth:
Merge:
```{r merge_samples_ps_Depth}
...
## Acantharian Sequence Percentage scatter plots (for comparison with imaging results)
...
```{r load_more_packages, message=FALSE}
library(readr)
library(reshape2)
...
p <-imagesizedepth_all %>% dplyr::select(pos_depth, size) %>%
  ...
  # Add a new column called 'bin': cut the depths into 10 m bins
